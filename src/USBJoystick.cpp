...
Joystick_::Joystick_(
	...
	bool includeBrake,
	bool includeSteering) : USBHID(get_usb_phy(), 0, 0, 0xDEAD, 0xBEEF, 0x1234)
{
	customHidReportDescriptor = (uint8_t *)malloc(256);
...
const uint8_t *Joystick_::report_desc()
{
	...
	if (_includeAxisFlags & JOYSTICK_INCLUDE_X_AXIS == JOYSTICK_INCLUDE_X_AXIS)
		axisCount++;
	if (_includeAxisFlags & JOYSTICK_INCLUDE_Y_AXIS == JOYSTICK_INCLUDE_Y_AXIS)
		axisCount++;
	if (_includeAxisFlags & JOYSTICK_INCLUDE_Z_AXIS == JOYSTICK_INCLUDE_Z_AXIS)
		axisCount++;
	if (_includeAxisFlags & JOYSTICK_INCLUDE_RX_AXIS == JOYSTICK_INCLUDE_RX_AXIS)
		axisCount++;
	if (_includeAxisFlags & JOYSTICK_INCLUDE_RY_AXIS == JOYSTICK_INCLUDE_RY_AXIS)
		axisCount++;
	if (_includeAxisFlags & JOYSTICK_INCLUDE_RZ_AXIS == JOYSTICK_INCLUDE_RZ_AXIS)
		axisCount++;

	uint8_t simulationCount = 0;
	if (_includeAxisFlags & JOYSTICK_INCLUDE_RUDDER == JOYSTICK_INCLUDE_RUDDER)
		simulationCount++;
	if (_includeAxisFlags & JOYSTICK_INCLUDE_THROTTLE == JOYSTICK_INCLUDE_THROTTLE)
		simulationCount++;
	if (_includeAxisFlags & JOYSTICK_INCLUDE_ACCELERATOR == JOYSTICK_INCLUDE_ACCELERATOR)
		simulationCount++;
	if (_includeAxisFlags & JOYSTICK_INCLUDE_BRAKE == JOYSTICK_INCLUDE_BRAKE)
		simulationCount++;
	if (_includeAxisFlags & JOYSTICK_INCLUDE_STEERING == JOYSTICK_INCLUDE_STEERING)
		simulationCount++;
	...
	if (simulationCount > 0)
	{
		...
		if (_includeAxisFlags & JOYSTICK_INCLUDE_RUDDER)
		{
			// USAGE (Rudder)
			tempHidReportDescriptor[hidReportDescriptorSize++] = 0x09;
		...
		if (_includeAxisFlags & JOYSTICK_INCLUDE_THROTTLE)
		{
			// USAGE (Throttle)
			tempHidReportDescriptor[hidReportDescriptorSize++] = 0x09;
		...
		if (_includeAxisFlags & JOYSTICK_INCLUDE_ACCELERATOR)
		{
			// USAGE (Accelerator)
			tempHidReportDescriptor[hidReportDescriptorSize++] = 0x09;
		...
		if (_includeAxisFlags & JOYSTICK_INCLUDE_BRAKE)
		{
			// USAGE (Brake)
			tempHidReportDescriptor[hidReportDescriptorSize++] = 0x09;
		...
		if (_includeAxisFlags & JOYSTICK_INCLUDE_STEERING)
		{
			// USAGE (Steering)
			tempHidReportDescriptor[hidReportDescriptorSize++] = 0x09;
	...
	// Create a copy of the HID Report Descriptor in a permanent place
	memcpy(customHidReportDescriptor, &tempHidReportDescriptor[0], hidReportDescriptorSize);

	// Calculate HID Report Size
	_hidReportSize = 1; // report ID
	...
	reportLength = hidReportDescriptorSize;

	return customHidReportDescriptor;
}
...
const uint8_t *Joystick_::configuration_desc(uint8_t index)
{
	if (index != 0)
	{
		return NULL;
...
void Joystick_::sendState()
{
	...
	// copy into the format the API wants to see
	HID_REPORT report;
	report.length = _hidReportSize;
	memcpy(&report.data, &data, sizeof(data));
